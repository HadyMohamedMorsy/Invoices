...
@section('content')
				<!-- row -->
				<div class="row">
					<form class="col-lg-12 col-md-12 form_category" action={{ route('catagories.store') }} method="POST" enctype="multipart/form-data">
						@csrf
						{{ method_field('POST') }}
						<div class="card">
							<div class="card-body">
								<div class="mb-4">
                                    <div class="form-group name_category">
                                        ...
                                        <input class="form-control text_category" type="text" placeholder="{{ __('categories.Name Of category') }}" data-name="name_cat"  name="name_cat_en">
                                    </div>
								</div>
...
<script>
	...
	saveData.addEventListener("click", (e) => {
		...
		if(target.dataset.auto == "noAuto"){
			...
			formApplication.setAttribute('action' , 'catagories/store');
